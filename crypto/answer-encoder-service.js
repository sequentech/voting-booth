    ...
    function(DeterministicJsonStringifyService) 
    {
      ...
      return function (requestedCodec, numAvailableOptions) 
      {
        var multi = {
           ...
           * [0, 2] -> ['01', '03']
           *
           * We need to be sure that each option will always be encoded using 
           ...
           * that we can codify any of those options with 2 characters, but if 
           * we have 110 options we need three chars per option.
           *
           * Next step is to concatenate the ordered list of option strings:
           *
           * [0, 2] -> ['01', '03'] -> '0103' -> 103
           *
           * NOTE: the zeros at the left of the final number are removed, 
           * because a number representation never has any zeros at the left. 
           * When doing decoding of the result you should take this into 
           * account.
           */
          ...
          {
            // first, convert the question answers into a list of selected 
            // options get only selected
        ...
        var pairwise = _.extend(
          _.clone(multi), 
          {
            ...
            extractAnswers: function(question) 
            {
              ...
              if (
                answers.length > question.max*2 ||
                answers.length < question.min*2
              ) {
                throw "error in the number of selected answers";
              }
